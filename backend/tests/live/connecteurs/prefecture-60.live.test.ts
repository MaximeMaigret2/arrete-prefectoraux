 ...
 * STRUCTURELLE pour `prefecture-60` (Le préfet de l'Oise), jamais
 ...
 *
 * D'après `configs/prefecture-60.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-60.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-60 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-60 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-60 (pas de PDF direct dans la liste).').not.toBeNull();
