...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-23`, jamais
 ...
 *
 * Préfecture de la Creuse — navigation à DEUX niveaux (racine → année →
 * catégorie « Spéciaux »). Page finale liste directement tous les
 * bulletins en `<div><a class="fr-link fr-link--download">` avec PDF
 * DIRECT (pas de `page_detail`, cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-23.yaml`
 * tient encore. N'affirme RIEN sur le CONTENU (titres, dates, nombre exact
 * de bulletins) — en particulier n'affirme PAS que l'arrêté anti rave-party
 * réel trouvé le 2026-08-14 (cf. SOURCE.md) est toujours listé (il expire
 * le 17/08/2026 et sera remplacé/retiré au fil du temps).
 */

const __dirname = path.dirname(fileURLToPath(import.meta.url));

async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-23.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-23 (manuel uniquement)', () => {
  it('la navigation à 2 niveaux (racine → année → Spéciaux) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-23 (PDF direct dans le bloc).').toBeNull();
