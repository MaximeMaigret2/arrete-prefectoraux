 ...
 * `prefecture-07`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel.
 *
 * Préfecture de l'Ardèche — 2 niveaux de navigation (année → mois, sans suffixe d'année dans l'URL du mois), liste finale non paginée (fr-downloads-group). Le lien PDF réel doit être cherché via l'attribut href (a[href$='.pdf']) à cause d'un bug de markup connu sur cette page (id= sans valeur casse class) — si ce test échoue sur "aucun lien PDF trouvé" après un déploiement, vérifier D'ABORD si ce bug a été corrigé côté site avant de suspecter une dérive de structure.
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-07.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-07.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-07 (V011, manuel uniquement)', () => {
