 ...
 * `prefecture-10`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel.
 *
 * Préfecture de l'Aube — 1 seul niveau de navigation (racine → liste annuelle DÉJÀ plate, déjà en ordre chronologique décroissant). point_acces corrigé par rapport au registre des sources initial (redirection serveur observée, cf. registre-sources.yaml).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-10.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-10.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-10 (V011, manuel uniquement)', () => {
