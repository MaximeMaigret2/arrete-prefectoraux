 ...
 * `prefecture-37`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site de la préfecture
 * d'Indre-et-Loire.
 *
 * IMPORTANT : ce connecteur a été déployé SANS capture live cette session
 * (fetch web bloqué sur tous les sous-chemins du domaine, cf. SOURCE.md) —
 * ce test est donc la PREMIÈRE vérification réelle de sa structure. Un
 * échec ici est attendu tant qu'aucune session n'a pu confirmer la
 * structure DOM réelle.
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-37.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-37.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-37 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-37 (PDF direct dans le div).').toBeNull();
