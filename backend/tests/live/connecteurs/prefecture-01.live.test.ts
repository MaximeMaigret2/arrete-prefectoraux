 ...
 * `prefecture-01`, jamais exécuté automatiquement (cf.
 ...
 * l'Ain — 2 niveaux de `navigation` (année → mois), comme prefecture-33,
 * mais liste finale PLATE (comme prefecture-13).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-01.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-01.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-01 (manuel uniquement)', () => {
