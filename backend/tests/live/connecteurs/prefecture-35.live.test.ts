 ...
 * `prefecture-35`, jamais exécuté automatiquement (cf.
 ...
 * d'Ille-et-Vilaine.
 *
 * Préfecture d'Ille-et-Vilaine — navigation à UN SEUL niveau (racine →
 * année, déjà la liste complète). Page de l'année liste des
 * `div[class=""]:has(a.fr-link--download)` avec lien PDF DIRECT (cf.
 * SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-35.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-35.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-35 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-35 (PDF direct dans le div).').toBeNull();
