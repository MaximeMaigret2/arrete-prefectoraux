 ...
 * STRUCTURELLE pour `prefecture-50` (Le préfet de la Manche), jamais
 ...
 *
 * D'après `configs/prefecture-50.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 3 niveaux de navigation, navigation par périodes (`periodes`, motif dépendant du mois courant, pas un simple gabarit `{mois_fr}`), au moins une étape `optionnelle: true` (absence de lien légitime, ex. pagination "dernière page").
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-50.yaml');
...
describe('Dérive structurelle — prefecture-50 (manuel uniquement)', () => {
  it('la navigation (3 niveaux de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    for (const [i, etape] of config.navigation.entries()) {
      ...
      if (etape.periodes) {
        ...
        expect(periode, `Navigation étape ${i} : aucune période ne couvre le mois courant (${moisCourant}) — config incomplète.`).toBeDefined();
        motif = substituerPlaceholders(periode!.motif, maintenant);
      } else {
        motif = substituerPlaceholders(etape.pattern_lien!, maintenant);
      }
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-50 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-50 (pas de PDF direct dans la liste).').not.toBeNull();
