 ...
 * STRUCTURELLE pour `prefecture-86` (Le préfet de la Vienne), jamais
 ...
 *
 * D'après `configs/prefecture-86.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : aucun niveau de navigation (url_liste résout directement la liste).
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-86.yaml');
...
describe('Dérive structurelle — prefecture-86 (manuel uniquement)', () => {
  it('la navigation (aucun niveau de navigation (url_liste résout directement la liste)) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-86 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-86 (pas de PDF direct dans la liste).').not.toBeNull();
