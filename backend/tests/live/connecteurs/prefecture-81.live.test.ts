 ...
 * STRUCTURELLE pour `prefecture-81` (Le préfet du Tarn), jamais
 ...
 *
 * D'après `configs/prefecture-81.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 3 niveaux de navigation, page de détail intermédiaire (`page_detail`, attribut_lien: "href") avant d'atteindre le PDF réel.
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-81.yaml');
...
describe('Dérive structurelle — prefecture-81 (manuel uniquement)', () => {
  it('la navigation (3 niveaux de navigation) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-81 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-81 (pas de PDF direct dans la liste).').not.toBeNull();
