 ...
 * STRUCTURELLE pour `prefecture-87` (Le préfet de la Haute-Vienne), jamais
 ...
 *
 * D'après `configs/prefecture-87.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 1 niveau de navigation, navigation par périodes (`periodes`, motif dépendant du mois courant, pas un simple gabarit `{mois_fr}`).
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-87.yaml');
...
describe('Dérive structurelle — prefecture-87 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-87 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-87 (pas de PDF direct dans la liste).').not.toBeNull();
