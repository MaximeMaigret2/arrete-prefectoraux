 ...
 * STRUCTURELLE pour `prefecture-71` (Le préfet de Saône-et-Loire), jamais
 ...
 *
 * D'après `configs/prefecture-71.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : aucun niveau de navigation (url_liste résout directement la liste), page de détail intermédiaire (`page_detail`, attribut_lien: "href") avant d'atteindre le PDF réel.
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-71.yaml');
...
describe('Dérive structurelle — prefecture-71 (manuel uniquement)', () => {
  it('la navigation (aucun niveau de navigation (url_liste résout directement la liste)) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-71 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-71 (pas de PDF direct dans la liste).').not.toBeNull();
