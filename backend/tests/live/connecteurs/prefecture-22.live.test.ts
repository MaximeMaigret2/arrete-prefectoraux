...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-22`, jamais
 ...
 *
 * Préfecture des Côtes-d'Armor — navigation à UN SEUL niveau (racine →
 * mois, sans carte année intermédiaire). Page du mois liste des cartes
 * DSFR complètes avec lien PDF DIRECT (`h2.fr-card__title a`, pas de
 * `page_detail`, cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-22.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-22.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-22 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → mois) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-22 (PDF direct dans la carte).').toBeNull();
