 ...
 * STRUCTURELLE pour `prefecture-48` (Le préfet de la Lozère), jamais
 ...
 *
 * D'après `configs/prefecture-48.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 3 niveaux de navigation, navigation par périodes (`periodes`, motif dépendant du mois courant, pas un simple gabarit `{mois_fr}`).
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-48.yaml');
...
describe('Dérive structurelle — prefecture-48 (manuel uniquement)', () => {
  it('la navigation (3 niveaux de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-48 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-48 (pas de PDF direct dans la liste).').not.toBeNull();
