...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-21`, jamais
 ...
 *
 * Préfecture de la Côte-d'Or — navigation à UN SEUL niveau (racine → année
 * en cours, désambiguïsation d'une racine à deux cartes, comme
 * prefecture-2A). Page finale liste directement tous les bulletins en
 * `<li><a class="fr-link fr-link--download">` avec PDF DIRECT (pas de
 * `page_detail`, cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-21.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-21.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-21 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année en cours) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-21 (PDF direct dans la liste).').toBeNull();
