 ...
 * `prefecture-12`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel.
 *
 * Préfecture de l'Aveyron — 2 niveaux de navigation (année "Recueils-2026"
 * → mois), liste finale non paginée (fr-downloads-group). Le lien PDF réel
 * doit être cherché via l'attribut href (a[href$='.pdf']) à cause d'un bug
 * de markup connu sur cette page (id= sans valeur casse class).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-12.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-12.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-12 (manuel uniquement)', () => {
