...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-25`, jamais
 ...
 * site de la préfecture du Doubs.
 *
 * Préfecture du Doubs — navigation à UN SEUL niveau (racine → année). Page
 * finale liste directement tous les bulletins en `<div><a class="fr-link
 * fr-link--download">` avec PDF DIRECT (pas de `page_detail`, cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-25.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-25.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-25 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-25 (PDF direct dans le bloc).').toBeNull();
