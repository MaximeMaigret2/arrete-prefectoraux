 ...
 * STRUCTURELLE pour `prefecture-70` (Le préfet de la Haute-Saône), jamais
 ...
 *
 * D'après `configs/prefecture-70.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-70.yaml');
...
describe('Dérive structurelle — prefecture-70 (manuel uniquement)', () => {
  it('la navigation (aucun niveau de navigation (url_liste résout directement la liste)) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-70 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-70 (pas de PDF direct dans la liste).').not.toBeNull();
