 ...
 * STRUCTURELLE pour `prefecture-91` (La préfète de l'Essonne), jamais
 ...
 *
 * D'après `configs/prefecture-91.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-91.yaml');
...
describe('Dérive structurelle — prefecture-91 (manuel uniquement)', () => {
  it('la navigation (2 niveaux de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-91 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-91 (pas de PDF direct dans la liste).').not.toBeNull();
