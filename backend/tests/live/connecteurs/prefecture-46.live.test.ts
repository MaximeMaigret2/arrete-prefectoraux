 ...
 * STRUCTURELLE pour `prefecture-46` (La préfète du Lot), jamais
 ...
 *
 * D'après `configs/prefecture-46.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 2 niveaux de navigation, au moins une étape `optionnelle: true` (absence de lien légitime, ex. pagination "dernière page"), page de détail intermédiaire (`page_detail`, attribut_lien: "href") avant d'atteindre le PDF réel.
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-46.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-46 (manuel uniquement)', () => {
  it('la navigation (2 niveaux de navigation) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-46 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-46 (pas de PDF direct dans la liste).').not.toBeNull();
