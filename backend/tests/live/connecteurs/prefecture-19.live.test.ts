...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-19`, jamais
 ...
 * de la préfecture de la Corrèze.
 *
 * Préfecture de la Corrèze — navigation à UN SEUL niveau (racine → année),
 * la plus courte observée à ce jour parmi les 21 connecteurs. Page de
 * l'année liste directement tous les bulletins en `<li><a class="fr-link
 * fr-link--download">` avec PDF DIRECT (pas de `page_detail`, cf.
 * SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-19.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-19.yaml');
...
describe('Dérive structurelle — prefecture-19 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-19 (PDF direct dans la liste).').toBeNull();
