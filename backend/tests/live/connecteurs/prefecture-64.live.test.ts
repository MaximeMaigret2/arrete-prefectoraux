 ...
 * STRUCTURELLE pour `prefecture-64` (Le préfet des Pyrénées-Atlantiques), jamais
 ...
 *
 * D'après `configs/prefecture-64.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 3 niveaux de navigation, au moins une étape `optionnelle: true` (absence de lien légitime, ex. pagination "dernière page").
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-64.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-64 (manuel uniquement)', () => {
  it('la navigation (3 niveaux de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-64 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-64 (pas de PDF direct dans la liste).').not.toBeNull();
