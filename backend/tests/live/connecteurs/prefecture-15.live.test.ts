 ...
 * `prefecture-15`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel — via l'URL
 * corrigée (casse "Actes-Administratifs", cf. registre-sources.yaml/SOURCE.md).
 *
 * Préfecture du Cantal — 1 seul niveau de navigation (racine → année, liste
 * finale déjà plate, comme prefecture-06/10), même lien portant titre et
 * URL PDF (`.fr-card__title a`).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-15.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-15.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-15 (manuel uniquement)', () => {
