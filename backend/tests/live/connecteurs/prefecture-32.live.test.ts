 ...
 * `prefecture-32`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site de la préfecture du
 * Gers.
 *
 * Préfecture du Gers — navigation à UN SEUL niveau (racine → année, déjà la
 * liste complète). Page de l'année est un VRAI `<table>` HTML listant des
 * `div[class=""]:has(a.fr-link--download)` avec lien PDF DIRECT (cf.
 * SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-32.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-32.yaml');
...
describe('Dérive structurelle — prefecture-32 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-32 (PDF direct dans le div).').toBeNull();
