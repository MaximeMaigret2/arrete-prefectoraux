 ...
 * STRUCTURELLE pour `prefecture-28`, jamais exécuté automatiquement (cf.
 ...
 * d'Eure-et-Loir.
 *
 * Préfecture d'Eure-et-Loir — navigation à DEUX niveaux (racine → année →
 * mois). Page du mois liste des `div[class=""]:has(a.fr-link--download)`
 * avec lien PDF DIRECT (cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-28.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-28.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-28 (manuel uniquement)', () => {
  it('la navigation à 2 niveaux (racine → année → mois) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-28 (PDF direct dans le div).').toBeNull();
