 ...
 * `prefecture-11`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel.
 *
 * Préfecture de l'Aude — 2 niveaux de navigation (année → mois, sans
 * suffixe d'année sur le lien du mois, comme prefecture-07), liste finale
 * non paginée (fr-downloads-group). Le lien PDF réel doit être cherché via
 * l'attribut href (a[href$='.pdf']) à cause d'un bug de markup connu sur
 * cette page (id= sans valeur casse class).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-11.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-11.yaml');
...
describe('Dérive structurelle — prefecture-11 (manuel uniquement)', () => {
