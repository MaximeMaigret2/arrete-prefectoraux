 ...
 * STRUCTURELLE pour `prefecture-30`, jamais exécuté automatiquement (cf.
 ...
 * Gard.
 *
 * Préfecture du Gard — navigation à UN SEUL niveau (racine → année, déjà la
 * liste complète). Page de l'année liste des
 * `div[class=""]:has(a.fr-link--download)` avec lien PDF DIRECT (cf.
 * SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-30.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-30.yaml');
...
describe('Dérive structurelle — prefecture-30 (manuel uniquement)', () => {
  it('la navigation à 1 seul niveau (racine → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-30 (PDF direct dans le div).').toBeNull();
