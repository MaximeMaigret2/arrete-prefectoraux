 ...
 * STRUCTURELLE pour `prefecture-79` (La préfète des Deux-Sèvres), jamais
 ...
 *
 * D'après `configs/prefecture-79.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 2 niveaux de navigation, navigation via `<select>` (`attribut_lien` autre que `href`, ex. `value` d'un `<option>`).
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-79.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-79 (manuel uniquement)', () => {
  it('la navigation (2 niveaux de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-79 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-79 (pas de PDF direct dans la liste).').not.toBeNull();
