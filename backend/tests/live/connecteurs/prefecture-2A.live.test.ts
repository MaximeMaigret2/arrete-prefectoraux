...
/**
 * (2026-08-14) — test de DÉRIVE STRUCTURELLE pour `prefecture-2A`, jamais
 ...
 * site de la préfecture de la Corse-du-Sud.
 *
 * Préfecture de la Corse-du-Sud — navigation à 2 niveaux (racine → recueil
 * de la préfecture de la Corse-du-Sud, désambiguïsé de la carte "Région
 * Corse" → année). Page de l'année liste directement tous les bulletins en
 * `<li><a class="fr-link fr-link--download">` avec PDF DIRECT (pas de
 * `page_detail`, cf. SOURCE.md).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-2A.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-2A.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-2A (manuel uniquement)', () => {
  it('la navigation à 2 niveaux (racine → recueil-préfecture → année) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    expect(config.page_detail, 'aucun page_detail attendu pour prefecture-2A (PDF direct dans la liste).').toBeNull();
