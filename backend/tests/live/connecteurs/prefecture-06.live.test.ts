 ...
 * `prefecture-06`, jamais exécuté automatiquement (cf.
 ...
 * Alpes-Maritimes — 2 niveaux de `navigation` (année → mois), comme
 * prefecture-01/33, liste finale PLATE (comme prefecture-13), page déjà en
 * ordre chronologique décroissant (pas d'étape "dernière page").
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-06.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-06.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-06 (V011, manuel uniquement)', () => {
