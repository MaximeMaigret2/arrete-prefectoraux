 ...
 * `prefecture-03`, jamais exécuté automatiquement (cf.
 ...
 * l'Allier — un seul niveau de `navigation` (racine → carte année), dont la
 * cible redirige serveur-side vers la liste complète (`fetch` suit la
 * redirection nativement).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-03.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-03.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-03 (manuel uniquement)', () => {
  it('la navigation (racine → année, redirection serveur suivie par fetch) résout une page finale exposant au moins un lien PDF', async () => {
