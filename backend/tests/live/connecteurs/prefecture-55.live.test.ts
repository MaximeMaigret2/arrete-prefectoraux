 ...
 * STRUCTURELLE pour `prefecture-55` (La préfète de la Meuse), jamais
 ...
 *
 * D'après `configs/prefecture-55.yaml` (config déjà validée par capture
 * live au moment du développement de ce connecteur) : 1 niveau de navigation, navigation via `<select>` (`attribut_lien` autre que `href`, ex. `value` d'un `<option>`).
 *
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-55.yaml');
...
describe('Dérive structurelle — prefecture-55 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-55 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-55 (pas de PDF direct dans la liste).').not.toBeNull();
