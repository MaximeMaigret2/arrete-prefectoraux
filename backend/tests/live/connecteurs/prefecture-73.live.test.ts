 ...
 * STRUCTURELLE pour `prefecture-73` (La préfète de la Savoie), jamais
 ...
 *
 * D'après `configs/prefecture-73.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-73.yaml');
  const raw = await readFile(configPath, 'utf-8');
  return PageWebConfigSchema.parse(yaml.load(raw));
}

describe('Dérive structurelle — prefecture-73 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-73 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-73 (pas de PDF direct dans la liste).').not.toBeNull();
