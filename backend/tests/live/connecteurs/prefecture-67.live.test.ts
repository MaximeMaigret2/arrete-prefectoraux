 ...
 * STRUCTURELLE pour `prefecture-67` (Le préfet du Bas-Rhin), jamais
 ...
 *
 * D'après `configs/prefecture-67.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-67.yaml');
...
describe('Dérive structurelle — prefecture-67 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page exposant des publications, et au moins une mène à une page de détail avec un lien PDF', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-67 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-67 (pas de PDF direct dans la liste).').not.toBeNull();
