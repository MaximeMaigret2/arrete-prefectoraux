 ...
 * STRUCTURELLE pour `prefecture-66` (Le préfet des Pyrénées-Orientales), jamais
 ...
 *
 * D'après `configs/prefecture-66.yaml` (config déjà validée par capture
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-66.yaml');
...
describe('Dérive structurelle — prefecture-66 (manuel uniquement)', () => {
  it('la navigation (1 niveau de navigation) résout une page finale exposant au moins un lien PDF direct', async () => {
    ...
    if (config.page_detail === null) {
      expect(config.page_detail, 'aucun page_detail attendu pour prefecture-66 (PDF direct dans la publication).').toBeNull();
      ...
      expect(config.page_detail, 'page_detail attendu pour prefecture-66 (pas de PDF direct dans la liste).').not.toBeNull();
