 ...
 * `prefecture-09`, jamais exécuté automatiquement (cf.
 * `backend/vitest.live.config.ts` / `README.md`, `npm run test:live-drift`
 * uniquement). De vrais appels réseau vers le site réel.
 *
 * Préfecture de l'Ariège — 2 niveaux de navigation : un motif LITTÉRAL fixe (collection d'archives "à partir du 28 avril 2015", sans placeholder de date) puis le mois courant, liste finale non paginée (fr-downloads-group). Le lien PDF réel doit être cherché via l'attribut href (a[href$='.pdf']) à cause d'un bug de markup connu sur cette page (id= sans valeur casse class).
 *
 * Vérifie UNIQUEMENT que la structure attendue par `configs/prefecture-09.yaml`
...
async function chargerConfigReelle(): Promise<ReturnType<typeof PageWebConfigSchema.parse>> {
  const configPath = path.join(__dirname, '../../../src/connecteurs/configs/prefecture-09.yaml');
...
describe('Dérive structurelle — prefecture-09 (V011, manuel uniquement)', () => {
